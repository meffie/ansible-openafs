    ...
    if have_extended_keys:
        for e in keys:
            ...
            rc, out, err = module.run_command(
                [asetkey, 'add', 'rxkad_krb5', kvno, eno, keytab.name, service_principal])
            if rc != 0:
                module.fail_json(msg="asetkey add failed", rc=rc, out=out, err=err, keys=keys)
    else:
        # Legacy style. Only supports DES encryption keys. Version 1.6.5+ will
        # directly read non-DES keys from rxkad.keytab, so we just ignore them
        # here.
        for e in keys:
            if e['eno'] in DES_ENCTYPES:
                kvno = str(e['kvno'])
                rc, out, err = module.run_command(
                    [asetkey, 'add', kvno, keytab.name, service_principal])
                if rc != 0:
                    module.fail_json(msg="asetkey add failed", rc=rc, out=out, err=err, keys=keys)

    # Check for changes and return list of key version numbers.
